...
def interpret_command_and_update_labyrinth(node, labyrinth, proc_defs, x, y, o):
    ...
    while node_list:
        ...
        if isinstance(node, SequenceBlock):
            ...
            if node.name == "i":
                if node.condition == o or (check_barrier(labyrinth, x, y, o) and node.condition == "b"):
                    node_list.insert(0, node.true_case)
                else:
                    node_list.insert(0, node.false_case)
            elif node.name == "u":
                if (node.condition != o and node.condition != "b") or (node.condition == "b" and not check_barrier(labyrinth, x, y, o)):
...
def print_labyrinth(labyrinth, pause=0.1, verbose=False):
    if verbose:
        ...
        if file.endswith("in"):
            ...
            for result, answer in zip(results, answers):
                if not (result[0] == int(answer[0]) and result[1] == int(answer[1]) and result[2] == answer[2]):
                    print(result, answer)
                    raise Exception("Test case not correct.", file)
                else:
                    print("correct")
